...
def get_dataframe_popqa(args):
    ...
    if dataset == "popqa-triplet":
        ...
        if args.mprecision == "fp32":
            print("read full dataset and perform 8:2 split")
            ...
            print("using pre-split from alpha server")
            train_fpath = "data/alpha_PQA_trainset_seed6.csv"
            train_df = pd.read_csv(train_fpath)
            train_df.drop(["Unnamed: 0"], axis=1, inplace=True)

            valid_fpath = "data/alpha_PQA_validset_seed6.csv"
            valid_df = pd.read_csv(valid_fpath)
            valid_df.drop(["Unnamed: 0"], axis=1, inplace=True)
...
def train(epoch, tokenizer, model, devices, dataloader, optimizer, scheduler, gdscaler, mprecision):
    ...
    for step, data in enumerate(dataloader, 1):
        ...
        gdscaler.scale(loss).backward()
        gdscaler.step(optimizer)
        scheduler.step()
        gdscaler.update()
        optimizer.zero_grad()
...
def train_weighted(
    epoch, tokenizer, srclen, tgtlen, model, devices, train_df, sample_ratio, sample_weights, optimizer, scheduler, gdscaler, mprecision
):
    timestamp_0 = time.time()
    print("[epoch {:02d}] {}".format(
        epoch, datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
...
def get_gdscaler():
    gdscaler = torch.cuda.amp.GradScaler(enabled=True)
    return gdscaler
# =========================== Checkpoint Save Load =============================


def save_checkpoint(model, optimizer, scheduler, gdscaler, epoch, args, train_sample_weights=None, type="train"):
...
def load_checkpoint(model, optimizer, scheduler, gdscaler, checkpoint_path):
    checkpoint = torch.load(checkpoint_path)
    model.load_state_dict(checkpoint["model"])
    optimizer.load_state_dict(checkpoint["optimizer"])
    scheduler.load_state_dict(checkpoint["scheduler"])
    gdscaler.load_state_dict(checkpoint["gdscaler"])
    epoch = checkpoint["epoch"]
    train_sample_weights = checkpoint["train_sample_weights"]
    return model, optimizer, scheduler, gdscaler, epoch, train_sample_weights
...
def main(args):
    ...
    if wandb.run.resumed:  # wandb auto resume
        ...
        print("wandb auto resume using checkpoint {} ...".format(
            auto_checkpoint), end="")
        model, optimizer, scheduler, gdscaler, epoch, train_sample_weights = load_checkpoint(
            model, optimizer, scheduler, gdscaler, auto_checkpoint
        )
        epoch = epoch + 1
        if args.use_importance_sampling:
        ...
        model, optimizer, scheduler, gdscaler, epoch, train_sample_weights = load_checkpoint(
            model, optimizer, scheduler, gdscaler, args.manual_checkpoint
        )
        epoch = epoch + 1
        print("... load finish")
    ...
    if args.mode == "train":  # for train & test
        ...
        while epoch < args.nepoch:
            if args.use_importance_sampling:
                ...
                train(epoch, tokenizer, model, devices, train_dataloader,
                      optimizer, scheduler, gdscaler, args.mprecision)

            if epoch % args.comp_metric_interval == 0:
                exact_match_score, f1_score, result_df = validate(
                    epoch, tokenizer, model, devices, valid_dataloader, mode="valid", save_result=True
                )

                if exact_match_score > best_em_score or epoch == 0:
                    best_em_epoch = epoch
                    best_em_score = exact_match_score
                    save_checkpoint(model, optimizer, scheduler, gdscaler,
                                    epoch, args, train_sample_weights, type="best_em")

                if f1_score > best_f1_score or epoch == 0:
                    best_f1_epoch = epoch
                    best_f1_score = f1_score
                    save_checkpoint(model, optimizer, scheduler, gdscaler,
                                    epoch, args, train_sample_weights, type="best_f1")
                ...
                if EARLY_STOPPING:
                    if (
                        ...
                        print("no performance improvement for {} epoch, stopping...".format(
                            EARLY_STOPPING_N_EPOCH))
                        break
            ...
            save_checkpoint(model, optimizer, scheduler, gdscaler,
                            epoch, args, train_sample_weights, type="train")
    ...
    if args.mode == "train":
        ...
        model, optimizer, scheduler, gdscaler, epoch, train_sample_weights = load_checkpoint(
            model, optimizer, scheduler, gdscaler, os.path.join(
                args.out_dir, "best_em_checkpoint.pt")
        )
        print("...load finish")
        save_checkpoint(model, optimizer, scheduler, gdscaler,
                        epoch, args, train_sample_weights, type="eval")
        exact_match_score, f1_score, result_df = validate(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mprecision", default="fp32",
                        type=str, help="mix precision")
    ...
    # handle precision
    if args.mprecision == "fp32":
        args.mprecision = torch.float32
    elif args.mprecision == "fp16":
        args.mprecision = torch.float16
    elif args.mprecision == "bf16":
        args.mprecision = torch.bfloat16
    else:
        raise Exception(
            "invalid mix precision type {}".format(args.mprecision))
